...
public class Desround {
    ...
    public Desround(String p) {
        ...
        {
            for(int j=0;j<sbox[0][i].length;j++)
            {
                sbox[0][i][j]=s1[i][j];
                sbox[1][i][j]=s2[i][j];
                sbox[2][i][j]=s3[i][j];
    ...
    //*************************************************
    public String Pe(String p)
    {
        String pr="";
        int [] pe={16 ,7 ,20 ,21, 29 ,12, 28 ,17
...
,19, 13, 30, 6, 22, 11, 4 ,25};
    ...
    public String Sbox(String s)
    {
        ...
        for(int i=0;i<sbox.length;i++)
        {
            ...
            s1=s.substring(i*6,(i*6)+6);
           // System.out.println(s1);
            sr=s1.charAt(0)+""+s1.charAt(s1.length()-1);
            sc=s1.substring(1,s1.length()-1);
            value=sbox[i][toDecimal(sr)][toDecimal(sc)];
            v+=toBinary(value);

           // System.out.println("+"+toBinary(value));
        }
      //  System.out.println("Value :"+v.length());
        return v;
    }
    //**********************************
    public int toDecimal(String m)
    {
        int m1=0;
        for(int i=0;i<m.length();i++)
        {
            m1+=((int)Math.pow(2, m.length()-i-1))*Integer.parseInt(m.substring(i,i+1));
        }
        return m1;
    }
    public String toBinary(int x)
    {
       ...
       // System.out.println("Beforee :"+s.length());
        int l=s.length();
        for(int i=l;i<4;i++)
        {
            s="0"+s;
        }
              //  System.out.println("Aftar :"+s.length()+"\n");
